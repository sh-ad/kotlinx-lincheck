...
public class SeparateOperationThreadExecutionGenerator extends ExecutionGenerator {
    private final Random random = new Random(0);

    public SeparateOperationThreadExecutionGenerator(CTestConfiguration testConfiguration, CTestStructure testStructure) {
    ...
    public ExecutionScenario nextExecution() {
        ...
        while (!threadGens.isEmpty()) {
            for (Iterator<ThreadGen> it = threadGens.iterator(); it.hasNext(); ) {
                ...
                if (threadGen.iThread == 0) {
                    Actor separateOperation = parallelGroup.stream()
                            .filter(ActorGenerator::isSeparateOperationThread)
                            .findFirst().get().generate(0);
                    parallelExecution.get(threadGen.iThread).add(separateOperation);
                } else {
